...
class SocketServer {
    ...
    init() {
        // Each session gets a unique socket so orphaned background processes
        // from a killed session can't accidentally send commands to a new
        // session that happens to share the same app PID.
